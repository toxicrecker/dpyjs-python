...
import DiscordUtils # pip install DiscordUtils
import re
import datetime
import math
# DiscordUtils is made by me
...
class Music(commands.Cog):
    ...
    @commands.command(aliases=["j","connect","cn","ct"])
        ...
        if not channel:
            try:
                ...
                if voice and voice.is_connected() and not voice.is_playing():
                    ...
                    await ctx.send("I am singing in another channel, can't listen to your orders right now!")
                else:
                    await ctx.send(f"Joined **{channel}**")
                    voice = await channel.connect()
            except AttributeError:
                await ctx.send("Connect yourself to a voice channel first or tell me what channel to join")
        else:
            if voice and voice.is_connected() and not voice.is_playing():
                if ctx.author.voice and ctx.author.voice.channel == channel:
                    ...
                    await ctx.send(f"You are not in {channel}, so i will not join it")
            elif voice and voice.is_connected() and voice.is_playing():
                await ctx.send("I am singing in another channel, can't listen to your orders right now!")
            else:
                if ctx.author.voice and ctx.author.voice.channel == channel:
